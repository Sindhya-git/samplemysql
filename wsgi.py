...
def home_page():
 ...
 #form = LoadForm(request.form)
  ...
  values = 'Clothing'
  cur.execute("SELECT s.ITEM_NUMBER, s.DESCRIPTION,s.LONG_DESCRIPTION, s.SKU_ATTRIBUTE_VALUE1,s.SKU_ATTRIBUTE_VALUE2,p.LIST_PRICE,p.DISCOUNT FROM XXIBM_PRODUCT_SKU s INNER JOIN XXIBM_PRODUCT_PRICING p WHERE s.ITEM_NUMBER=p.ITEM_NUMBER LIMIT 4", (values,))
  shirts = cur.fetchall()
 # Close Connection
  cur.close()
